...
public class AddTeacher extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {  
            ...
            if (identity.equals(temp))  {
                ...
                add(res, username, password, identity); 
            }
    }  

	public void add(HttpServletResponse res, String username, String password, String identity) {  
		
		SqlBean db = new SqlBean(); 
        int row=0;

	 	String sql = "insert into user values('" + username + "','" + password + "','" + identity + "')";  

	 	try {  
	 		row = db.executeInsert(sql);  
	 		if(row == 1){
	 			JOptionPane.showConfirmDialog(null, "添加成功！","提示",JOptionPane.DEFAULT_OPTION);
	 			res.sendRedirect("add_teacher.jsp");
	 		}		
	 	} catch (Exception e) {  
     ...
     public String getIdentity(HttpServletRequest req, HttpServletResponse res, String username, String identity) throws ServletException, IOException {  
      ...
      public void doError(HttpServletRequest req, HttpServletResponse res, String str) throws ServletException, IOException {  
    	  ...
    	  RequestDispatcher rd = getServletContext().getRequestDispatcher("/add_teacher.jsp");  
    	  rd.forward(req, res);  
      }  
